    ...
    try {
      const eventData: EventRequest = {
        ...
        schedules: formData.schedules
          .filter(schedule => schedule.showDate && schedule.showTime)
          .map(schedule => ({
...
const addSchedule = () => {
  setFormData(prev => ({
    ...prev,
    schedules: [...prev.schedules, { showDate: "", showTime: "" }]
  }))
}
...
const updateSchedule = (index: number, field: 'showDate' | 'showTime', value: string) => {
  setFormData(prev => ({
    ...prev,
...
const handleEdit = (event: EventResponse) => {
  ...
  setFormData({
    ...
    schedules: event.schedule 
      ? [{ showDate: event.schedule.showDate, showTime: event.schedule.showTime?.substring(0, 5) || "" }]
      : [{ showDate: "", showTime: "" }],
  })
                      ...
                      {formData.schedules.map((schedule, index) => (
                        <div key={index} className="flex items-center gap-2 p-3 border rounded-lg">
                          <div className="flex-1">
                            <Label htmlFor={`showDate-${index}`} className="text-sm">공연일</Label>
                            <Input
                              id={`showDate-${index}`}
                              type="date"
                              value={schedule.showDate}
                              onChange={(e) => updateSchedule(index, 'showDate', e.target.value)}
                            />
                          </div>
                          <div className="flex-1">
                            <Label htmlFor={`showTime-${index}`} className="text-sm">공연시간</Label>
                            <Input
                              id={`showTime-${index}`}
                              type="time"
                              value={schedule.showTime}
                              onChange={(e) => updateSchedule(index, 'showTime', e.target.value)}
                            />
                          </div>
